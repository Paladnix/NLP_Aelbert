...
DATA_CONF = {
    "PAWS_TRAIN": {"filename": DATA_DIR + "/paws_train.csv", "sep": ","},
    "PAWS_DEV"  : {"filename": DATA_DIR + "/paws_dev.csv",   "sep": ","},
    "QQP_TRAIN" : {"filename": DATA_DIR + "/qqp_train.csv",  "sep": "\t"},
    "QQP_DEV"   : {"filename": DATA_DIR + "/qqp_dev.csv",    "sep": "\t"},
}
...
class PawsDataset(Dataset):
    ...
    def get_token_ids(self, row):
        ...
        s1 = row.sentence1
        s2 = row.sentence2
        ids_pair = self.tokenizer.encode(s1, s2, max_length=MAX_LEN, add_special_tokens=True, truncation=True, truncation_strategy="longest_first")
...
def get_loader(df, sep, batch_size=32, is_train=True):
    ds_df = PawsDataset(df, sep)
...
def get_train_loader(config):
    return get_loader(DATA_CONF[config.train_data]['filename'], sep=DATA_CONF[config.train_data]['sep'])

def get_test_loader(config):
    return get_loader(DATA_CONF[config.test_data]['filename'], is_train=False, sep=DATA_CONF[config.test_data]['sep'])
