...
'''
 1. padding PAD_TOKEN to right for length that less than SEQ_LENGTH
 2. save left SEQ_LENGTH if CUT_LEFT = True
 3. select Not PAD_TOKEN as OUTPUT_SEQ and  keep right SEQ_LENGTH for longger 
    or padding PAD_TOKEN to left. 
'''
...
class TypeDataset(object):
  ...
  def __init__(self, filepattern, vocab, goal, lstm_type):
    """
      ...
      vocab      : 词向量字典
      goal       : 目标数据集
      lstm_type  : lstm的类型，simple or two
    """
    self._all_shards = glob.glob(filepattern)   # 文件可能分片
    self.goal = goal
    ...
    logging.info("Answer num %d" % (self.answer_num))
