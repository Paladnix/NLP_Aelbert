...
class ModelArgs():
    ...
    def __init__(self):
        self.embeds_dim = 768
        self.linear_size = self.embeds_dim * 2
        self.num_layer = 3
        self.max_length = 64



class MulCNN3(nn.Module):
    def __init__(self, args = ModelArgs()):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.embeds_dim*(1+self.num_layer)*2, 1)
            # nn.LayerNorm(self.linear_size),
            # nn.ReLU(inplace=True), #   原本是True
...
class Wide_Conv(nn.Module):
    def __init__(self, seq_len, embeds_size):
        ...
        self.W = nn.Parameter(torch.ones([seq_len, embeds_size]))
...
def attention_avg_pooling(sent1, sent2):
    # A: batch_size * seq_len * seq_len
    A = match_score(sent1, sent2)
...
if __name__ == '__main__':
    s1 = torch.randn([2, 3, 4])
    s2 = torch.randn([2, 3, 4])
    a = match_score(s1, s2)
